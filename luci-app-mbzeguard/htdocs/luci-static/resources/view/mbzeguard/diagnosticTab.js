...
'require uci';
'require fs';
'require view.mbzeguard.constants as constants';
'require view.mbzeguard.utils as utils';
...
function checkDNSAvailability() {
    return new Promise(async (resolve) => {
        try {
            safeExec('/usr/bin/mbzeguard', ['check_dns_available'], 'P0_PRIORITY', dnsStatusResult => {
...
function showConfigModal(command, title) {
    ...
    try {
        ...
        if (command === 'global_check') {
            safeExec('/usr/bin/mbzeguard', [command], 'P0_PRIORITY', res => {
...
const ButtonFactory = {
    ...
    createActionButton: function (config) {
        return this.createButton({
            ...
            onClick: () => safeExec('/usr/bin/mbzeguard', [config.action], 'P0_PRIORITY')
    ...
    createInitActionButton: function (config) {
        return this.createButton({
            label: config.label,
            additionalClass: `cbi-button-${config.type || ''}`,
            onClick: () => safeExec('/etc/init.d/mbzeguard', [config.action], 'P0_PRIORITY')
                .then(() => config.reload && location.reload()),
            style: config.style
        });
    },
...
let createStatusSection = async function () {
    // Get initial mbzeguard status
    let initialmbzeguardStatus = { enabled: false };
    try {
        const result = await fs.exec('/usr/bin/mbzeguard', ['get_status']);
        if (result && result.stdout) {
            const status = JSON.parse(result.stdout);
            initialmbzeguardStatus.enabled = status.enabled === 1;
        }
    } catch (e) {
        console.error('Error getting initial mbzeguard status:', e);
    }

    return E('div', { 'class': 'cbi-section' }, [
        E('div', { 'class': 'table', style: 'display: flex; gap: 20px;' }, [
            // mbzeguard Status Panel
            E('div', { 'id': 'mbzeguard-status-panel', 'class': 'panel', 'style': 'flex: 1; padding: 15px;' }, [
                E('div', { 'class': 'panel-heading' }, [
                    E('strong', {}, _('mbzeguard Status')),
                    E('br'),
                    E('span', { 'id': 'mbzeguard-status-text' }, createLoadingStatusText())
                ]),
                E('div', { 'class': 'panel-body', 'style': 'display: flex; flex-direction: column; gap: 8px;' }, [
                    ButtonFactory.createActionButton({
                        label: 'Restart mbzeguard',
                        type: 'apply',
                        action: 'restart',
                        reload: true
                    }),
                    ButtonFactory.createActionButton({
                        label: 'Stop mbzeguard',
                        type: 'apply',
                        action: 'stop',
                        reload: true
                    }),
                    ...
                    ButtonFactory.createInitActionButton({
                        label: initialmbzeguardStatus.enabled ? 'Disable Autostart' : 'Enable Autostart',
                        type: initialmbzeguardStatus.enabled ? 'remove' : 'apply',
                        action: initialmbzeguardStatus.enabled ? 'disable' : 'enable',
                        reload: true
                    }),
            ...
            E('div', { 'id': 'version-info-panel', 'class': 'panel', 'style': 'flex: 1; padding: 15px;' }, [
                ...
                E('div', { 'class': 'panel-body' }, [
                    E('div', { 'style': 'margin-top: 10px; font-family: monospace; white-space: pre-wrap;' }, [
                        E('strong', {}, _('mbzeguard: ')), E('span', { 'id': 'mbzeguard-version' }, _('Loading...')), '\n',
...
async function updateDiagnostics() {
    // mbzeguard Status check
    safeExec('/usr/bin/mbzeguard', ['get_status'], 'P0_PRIORITY', result => {
        try {
            const parsedmbzeguardStatus = JSON.parse(result.stdout || '{"enabled":0,"status":"error"}');

            // Update mbzeguard status text
            updateTextElement('mbzeguard-status-text',
                E('span', {
                    'style': `color: ${parsedmbzeguardStatus.enabled ? constants.STATUS_COLORS.SUCCESS : constants.STATUS_COLORS.ERROR}`
                }, [
                    parsedmbzeguardStatus.enabled ? '✔ Autostart enabled' : '✘ Autostart disabled'
                ])
            );

            // Update autostart button
            const autostartButton = parsedmbzeguardStatus.enabled ?
                ButtonFactory.createInitActionButton({
            ...
            const panel = document.getElementById('mbzeguard-status-panel');
            if (panel) {
    ...
    safeExec('/usr/bin/mbzeguard', ['get_sing_box_status'], 'P0_PRIORITY', result => {
        try {
    ...
    safeExec('/usr/bin/mbzeguard', ['show_version'], 'P2_PRIORITY', result => {
        updateTextElement('mbzeguard-version',
            document.createTextNode(result.stdout ? result.stdout.trim() : _('Unknown'))
        );
    });

    safeExec('/usr/bin/mbzeguard', ['show_luci_version'], 'P2_PRIORITY', result => {
        updateTextElement('luci-version',
            document.createTextNode(result.stdout ? result.stdout.trim() : _('Unknown'))
        );
    });

    safeExec('/usr/bin/mbzeguard', ['show_sing_box_version'], 'P2_PRIORITY', result => {
        updateTextElement('singbox-version',
            document.createTextNode(result.stdout ? result.stdout.trim() : _('Unknown'))
        );
    });

    safeExec('/usr/bin/mbzeguard', ['show_system_info'], 'P2_PRIORITY', result => {
        if (result.stdout) {
    ...
    runAsyncTask(async () => {
        try {
            ...
            const data = await uci.load('mbzeguard');
            const proxyString = uci.get('mbzeguard', 'main', 'proxy_string');
...
function setupDiagnosticsEventHandlers(node) {
    const titleDiv = E('h2', { 'class': 'cbi-map-title' }, _('mbzeguard'));
    ...
    document.addEventListener('visibilitychange', function () {
        ...
        if (document.hidden || !diagnosticsTab || !diagnosticsTab.classList.contains('cbi-tab-active')) {
            ...
            // Don't stop error polling here - it's managed in mbzeguard.js for all tabs
